const { getAttributeNameCompletions, getAttributeValueCompletions } = require('./helpers');
...
module.exports = function({ editor, bufferPosition }) {
  ...
  switch (node.type) {
    ...
    case "'": {
      ...
      const tagNameNode = containerNode.descendantsOfType('tag_name')[0];

      // Get the last attribute name before the quote
      const attributeNameNode = containerNode
        .descendantsOfType('attribute_name', null, node.startPosition)
        .pop();
...
function tokenBeforePosition(editor, position) {
  ...
  while (position.isLessThan(node.endPosition) || node.isMissing() || node.type === 'text') {
