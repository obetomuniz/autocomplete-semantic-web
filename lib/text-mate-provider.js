...
function hasStringScope(scopes) {
  return (
    scopes.includes('string.quoted.double.html') || scopes.includes('string.quoted.single.html')
  );
}
...
function getPreviousAttribute(editor, bufferPosition) {
  ...
  const match = attributePattern.exec(
    editor.getTextInRange([[bufferPosition.row, 0], [bufferPosition.row, quoteIndex]])
  );
